...
<div class="eolia_form-row eolia_form-row--<?php echo $field->get_component() ?><?php echo ! $field->is_mobile(
	...
	<div class="eolia_form-group<?php echo $field->get_required() ? ' eolia_form-group--required' : null ?>"
	     data-field-id="<?php echo $field->get_id() ?>">
		<p class="eolia_field_label<?php echo $field->get_required(
		) ? ' eolia_field_label--required' : null ?>"><?php echo $field->get_label() ?></p>
		<div class="eolia_input-group eolia_input-group--file">
			<label class="eolia_input eolia_input--button" for="<?php echo $field->get_id() ?>">
				<?php echo apply_filters(
					"eolia_filter_form_field_{$field->get_id()}_upload_button",
					_x( 'Browse...', 'form uploaded', 'eolia-app' ), eolia_get_job()
				) ?>
			</label>
			<input type="file" hidden
			       class="hidden-file" <?php echo FieldController::formatAttributes(
				$field->get_field_attributes()
			) ?>/>
			<input type="text" name="label_file_<?php echo $field->get_id() ?>"
			       class="eolia_input eolia_input--filelabel" readonly>
		</div>
		<?php if ( $options['is_applygoogle'] || $options['is_applydropbox'] || $options['is_applyviadeo'] || $options['is_applylinkedin'] || $options['is_applyindeed'] ) : ?>
			<div class="eolia_input-group eolia_input-group--file-advanced">
				<?php if ( $options['is_applygoogle'] ) : ?>
					...
					<a href="#" class="eolia_input eolia_input--button upload_with upload_with--googledrive"
					   data-ga="Offer::Upload::Google Drive" data-replace="<?php echo $field->get_id(); ?>">
						<svg class="fileUpload-picto fileUpload-picto--google" version="1.1"
				...
				<?php if ( 'pj1' === $field->get_id() ): ?>
					...
					<a title="<?php printf(
						...
						'viadeo'
					); ?>" href="#" class="eolia_input eolia_input--button apply_with apply_with--viadeo"
					   data-ga="Offer::Apply With::Viadeo" data-replace="<?php $field->get_id() ?>"
					   data-url-signin="https://partners.viadeo.com/oauth/authorize?client_id=<?php echo $options['viadeo_id'] ?>&redirect_uri=<?php echo urlencode(
						   plugins_url(
							   'App/Callbacks/viadeo_response.php',
							   'eolia-app/eolia-app.php'
						   )
					   ) ?>&response_type=code&scope=api&state=<?php echo $_SESSION['viadeo_state'] ?>"
					   data-url-check-auth="<?php echo plugins_url(
						   'App/Callbacks/viadeo_response.php',
						   'eolia-app/eolia-app.php'
					   ) ?>">
				...
				<?php $_SESSION['linkedin_state'] = substr( sha1( rand() ), 0, 20 ); ?>
					<a title="<?php printf(
						...
						'LinkedIn'
					) ?>" href="#" class="eolia_input eolia_input--button apply_with apply_with--linkedin"
					   data-ga="Offer::Apply With::Linkedin"
					   data-url-signin="https:/www.linkedin.com/oauth/v2/authorization?client_id=<?php echo $options['linkedin_id'] ?>&redirect_uri=<?php echo urlencode(
						   plugins_url(
							   'App/Callbacks/linkedin_response.php',
							   'eolia-app/eolia-app.php'
						   )
					   ) ?>&state=<?php echo $_SESSION['linkedin_state'] ?>&response_type=code&scope=r_basicprofile%20r_emailaddress"
					   data-url-check-auth="<?php echo plugins_url(
						   'App/Callbacks/linkedin_response.php',
						   'eolia-app/eolia-app.php'
					   ) ?>">
				...
				$post_url                  = get_bloginfo( 'url' ) . '?' . http_build_query(
						array(
							'action' => 'indeed',
							'id'     => session_id(),
						)
				...
				?>
					<div style="display: none">
						...
						<span class="indeed-apply-widget"
						      ...
						      data-indeed-apply-jobId="<?php echo $job->get_business_unit() . '_' . $job->get_id(
							      ) . '_' . $job->get_ref() ?>"
						      ...
						      data-indeed-apply-postUrl="<?php echo esc_url(
							      add_query_arg(
								      _x(
									      'apply', 'slug',
									      'eolia-app'
