...
<div class="eolia_form-row eolia_form-row--<?php echo $field->get_component() ?><?php echo ! $field->is_mobile() ? ' eolia_form-row--mobile-hidden' : null ?>" data-field-id="<?php echo $field->get_id() ?>">
	<div class="eolia_form-group<?php echo $field->get_required() ? ' eolia_form-group--required' : null ?>"
		...
		<select <?php echo FieldController::formatAttributes($field->get_field_attributes()) ?>>
			<option selected disabled></option>
			<?php foreach ($field->get_values() as $key => $value): ?>
				<option
					value="<?php echo $key ?>"<?php if (array_key_exists('parent_id',
					$value)): ?> data-parent-id="<?php echo $value['parent_id'] ?>"<?php endif ?><?php if(isset($_GET[$field->get_id()]) && $key == $_GET[$field->get_id()]) : ?> selected<?php endif ?>>
